...
from ...database.models import Req, User as DBUser, Module, Tag
...
async def list_requirements(
    ...
    if module_id:
        query = query.where(Req.module_id == module_id)
    
    if product_id:
        # Filter by product association - get requirements from modules associated with the product
        from ...database.models import ProductModule
        query = (
            query
            .join(ProductModule, ProductModule.module_id == Req.module_id)
...
async def create_requirement(
    ...
    # Validate module belongs to workspace
    module_query = select(Module).where(
        and_(
            Module.id == req_data.module_id,
            Module.workspace_id == workspace_id
        )
    )
...
async def create_requirement_version(
    ...
    new_version = Req(
        ...
        module_id=latest_version.module_id,
    ...
    requirement_response = RequirementResponse(
        ...
        module_id=new_version.module_id,
