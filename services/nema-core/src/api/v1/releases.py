...
from ...database.models import Release, Component
...
async def list_releases(
    ...
    # Apply filters
    if component_id:
        query = query.where(Release.component_id == component_id)
...
async def create_release(
    ...
    component = await get_component_in_workspace(workspace_id, release_data.component_id, db)
    
    # Get previous release for this component (if any)
    prev_release_query = (
        select(Release)
        .where(Release.component_id == release_data.component_id)
    ...
    new_release = Release(
        component_id=release_data.component_id,
    ...
    release_response = ReleaseResponse(
        id=new_release.id,
        component_id=new_release.component_id,
